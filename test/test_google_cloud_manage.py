...
def test_get_groups_for_user_success(test_cloud_manager):
    """
    Test that get_groups_for_user successfully returns a list of groups
    for a user.
    """
    # Setup
    user_email = "testuser@test-domain.com"
    group_1 = {
        "email": "group1@test-domain.com"
    }
    group_2 = {
        "email": "group2@test-domain.com"
    }

    # Mock the Google Admin SDK response
    mock_response = {
        "groups": [group_1, group_2]
    }

    test_cloud_manager._admin_service.groups.return_value.list.return_value.execute.return_value = mock_response

    # Call
    result = test_cloud_manager.get_groups_for_user(user_email)

    # Test
    assert result == ["group1@test-domain.com", "group2@test-domain.com"]
    test_cloud_manager._admin_service.groups.return_value.list.assert_called_once_with(userKey=user_email)


def test_get_groups_for_user_no_groups(test_cloud_manager):
    """
    Test that get_groups_for_user returns an empty list when no groups
    are found for the user.
    """
    # Setup
    user_email = "testuser@test-domain.com"

    # Mock the Google Admin SDK response with no groups
    mock_response = {
        "groups": []
    }
...
def test_get_groups_for_user_api_error(test_cloud_manager):
    """
    Test that get_groups_for_user handles API errors gracefully, including retries.
    """
    # Setup
    user_email = "testuser@test-domain.com"

    # Mock the Google Admin SDK to raise an HttpError multiple times
    test_cloud_manager._admin_service.groups.return_value.list.return_value.execute.side_effect = HttpError(
    ...
    # Call and Test
    with pytest.raises(HttpError):
        test_cloud_manager.get_groups_for_user(user_email)

    # Check that the list method was called multiple times (depending on your retry logic)
    assert test_cloud_manager._admin_service.groups.return_value.list.call_count > 1
    test_cloud_manager._admin_service.groups.return_value.list.assert_called_with(userKey=user_email)
